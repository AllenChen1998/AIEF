...
def G_synthesis(
    ...
    noise_inputs        = [],           # specificied noise input variables, modified by csz in 2019.9.29
    ...
    if noise_inputs == []:
        if use_noise:
            for layer_idx in range(num_layers):
                res = layer_idx // 2 + 2
                shape = [int(dlatents_in.shape[0]), 1, 2**res, 2**res]
                noise_inputs.append(tf.get_variable('noise%d' % layer_idx, shape=shape, initializer=tf.initializers.random_normal(), trainable=False))
    else:
        assert len(noise_inputs) == num_layers
        for layer_idx, noise in enumerate(noise_inputs):
            res = layer_idx // 2 + 2
            shape = [int(dlatents_in.shape[0]), 1, 2**res, 2**res]
            assert noise.shape == shape, str(noise.shape) + ' vs ' + str(shape)
        randomize_noise = False # priority: use_noise > noise_inputs > randomize_noise
