...
def convert_images_to_uint8(images, drange=[-1,1], nchw_to_nhwc=False, shrink=1, uint8_cast=True):
    ...
    return tf.saturate_cast(images, tf.uint8) if uint8_cast else images


def convert_images_to_01(images, drange=[-1,1], nchw_to_nhwc=False, name=None):
    """Convert a minibatch of images from float32 to uint8 with configurable dynamic range.
    Can be used as an output transformation for Network.run().
    """
    images = tf.cast(images, tf.float32)
    ...
    scale = 255 / (drange[1] - drange[0])
    images = images * scale + (0.5 - drange[0] * scale)
    zeros = tf.zeros_like(images)
    ones = tf.ones_like(images)
    images = tf.where(images < 0, zeros, images)
    images = tf.where(images > 255, 255.0*ones, images)
    return tf.cast(images, tf.float32, name=name) / 255.0
